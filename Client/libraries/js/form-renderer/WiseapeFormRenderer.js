...
var WiseapeFormRenderer = Class({
	...
	renderContainer: function(me, ctrl)
	{
		...
		switch(ctrl.type)
		{
			...
			case "formgroup":
				...
				if(ctrl.height == null)
					ctrl.height = "85%";
				html = "\n<div style=\"height:" + ctrl.height + ";width: 100%; padding: 0px;  overflow-y: auto; overflow-x: hidden\">{{content}}\n</div>\n";
				var controls = ctrl.controls;
				var md = Math.round(12 / controls.length);
				for(var i = 0; i < controls.length; i++)
				{
	...
	renderControl: function(me, ctrl, md)
	{
		...
		if(ctrl.rowClass == null)
			ctrl.rowClass = "col-md-" + md;

		var html = "\n\t<div class='" + ctrl.rowClass + "'>\n\t\t<div class='form-group'>\n\t\t\t<label for='" + ctrl.id + "'>" + ctrl.label + "</label>{{input}}\n\t\t</div>\n\t</div>";
		var input = "";
		...
		switch(ctrl.type)
		{
			...
			case "combobox":
				if(ctrl.width == null)
						ctrl.width = '100%';
				input = "\n\t\t\t<select control-type='combobox' id='" + ctrl.id + "' name='" + ctrl.id + "' data-field='" + ctrl.dataField + "' " + events + "  class='form-control select2' style='width:" + ctrl.width + "' >{{options}}</select>";
				if(ctrl.dataSource != null)
				{
					

					input = "\n\t\t\t<select control-type='combobox' id='" + ctrl.id + "' name='" + ctrl.id + "' data-field='" + ctrl.dataField + "' value-field='" + ctrl.dataSourceInfo.valueField + "' display-field='" + ctrl.dataSourceInfo.displayField + "' " + events + " class='form-control select2' style='width:" + ctrl.width + "' >{{options}}</select>";
	...
	createWindow: function( container, id, content, option)
	{
		...
		$("#" + container + "").append( "<div id=\"" + id + "\" style='padding-right: 0px; padding-left: 0px; overflow-x: hidden; padding: 12px'>" + "" + "</div>" );
		...
		$("#" + id).prev().append("<span id='btnClose_" + id + "' onclick='MEM." + id + ".close()' style=\"cursor: pointer;display:inline-block; width:30px; height:30px;float: right;vertical-align: middle;text-align:center; padding-top: 3px\"><i class=\"fa fa-times-circle\"></i></span>");
		$("#" + id).prev().append("<span id='btnMax_" + id + "'  onclick='MEM." + id + ".maximize()' onclick1='MEM." + id + ".maximize()' onclick2='MEM." + id + ".restore()' style=\"cursor: pointer;display:inline-block; width:30px; height:30px;float: right;vertical-align: middle;text-align:center; padding-top: 3px\"><i class=\"fa fa-square-o\"></i></span>");
		$("#" + id).prev().append("<span id='btnMin_" + id + "'  onclick='MEM." + id + ".minimize()' style=\"cursor: pointer;display:inline-block; width:30px; height:30px;float: right;vertical-align: middle;text-align:center; padding-top: 3px\"><i class=\"fa fa-minus\"></i></span>");
	...
	resizeWindow: function(win, w, h, showRestoreButton)
	{
		...
		if(showRestoreButton == 1)
		{
			...
			$("#btnMax_" + id + " > i").removeClass("fa-square-o");
			$("#btnMax_" + id + " > i").addClass("fa-expand");
		...
		{
			var id = $(win).attr("winname");
			$("#btnMax_" + id + " > i").addClass("fa-square-o");
			$("#btnMax_" + id + " > i").removeClass("fa-expand");
