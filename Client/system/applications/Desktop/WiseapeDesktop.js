var UserDisplaySetting =
{
	...
	theme: 
	{
		themeName: "Default",
		themeInfo: "",
		loadingAnimation: "images/hummingbird.gif",
		loadingAnimationAudio: "sound/bird.mp3",
		defaultWallpaper: "",
		taskbarColor: "",
		taskbarMenuColor: ""
	}
...
var WiseapeDesktop = Class(WiseapeApplication, {
	...
	run: function(client, appGroupConfigs)
	{
		...
		client.loadSystemPage("Desktop/html/index.html?id=" + Util.createId(5), function (html)
		{
			...
			console.log("loadUiRenderer Start");
			me.loadUiRenderer(me, UIConfig, function()
			{
				console.log("loadUiRenderer End");
				console.log("addCssAndScripts Start");
				me.addCssAndScripts(me, function()
				{
					console.log("addCssAndScripts End");
	...
	,
	displayTheme: function(me, callback)
	{
		var username = OAuthClient.readCookie("OAuthUsername");
		var url = me.client.serverConfig.webService + "/GlobalApplicationSettingService.svc/globalapplicationsetting/find-by-userid/" + username;
		WebClient.get(url, function(result)
		{
			var settings = result.Data;
			UserDisplaySetting.wallpaper = me.getSetting(settings, "DesktopWallpaper");
			var themeName = me.getSetting(settings, "Theme");
			url = me.client.serverConfig.webService + "/ThemeService.svc/theme/find-by-theme-name/" + themeName;
			console.log("Get Theme : " + url);
			WebClient.get(url, function(result)
			{
				var theme = result.Data[0];
				console.log("Theme: " );
				console.log(theme);
				UserDisplaySetting.theme.themeName = theme.ThemeName;
				UserDisplaySetting.theme.themeInfo = theme.ThemeInfo;
				UserDisplaySetting.theme.loadingAnimation = theme.LoadingAnimation;
				UserDisplaySetting.theme.loadingAnimationAudio = theme.LoadingAnimationAudio;
				UserDisplaySetting.theme.defaultWallpaper = theme.DefaultWallpaper;
				UserDisplaySetting.theme.taskbarColor = theme.TaskbarColor;
				UserDisplaySetting.theme.taskbarMenuColor = theme.TaskbarMenuColor;
				me.displayThemeDetail(me);
			});
			
		});
	}
	,
	displayThemeDetail: function(me)
	{
		//$(".left-panel").css("background-color", UserDisplaySetting.theme.taskbarColor);
		//$("#content-panel").css("background-url", UserDisplaySetting.theme.defaultWallpaper);
	}
	,
	...
	,
	displayRunningAppInTaskbar: function(me, appItem)
	{
		var ran = Math.round( Math.random(100000) * 100000 );
		var iconPath = appItem.ApplicationPath + "/" +  appItem.ApplicationIcon;
		var div = "<div id='taskbar-" + appItem.ApplicationName + ran + "' class='taskbar-icon' style='background: url(" + iconPath + ") no-repeat; background-size: 80% 80%; background-position: center'></div>";
		$("#app-icon-container").append(div);
		$("#taskbar-" + appItem.ApplicationName + ran + "").mouseover( function()
		{
	...
	displayApplicationGroups: function(client, appGroups)
	{
		...
		{	
			...
			var li = "<li><span><div class='menu-item-img-wrapper'><img src='" + appGroup.ApplicationGroupIcon + "' /></div><div class='menu-item-text-wrapper'>" + appGroup.ApplicationGroupName + "</div></span>{menuItems}</li>";
			var menuItems = $("<ul></ul>");
			for(var j = 0; j < appGroup.ApplicationItems.length; j++)
			{
				...
				{
					var appItemLi = $("<li appId='" + appItem.IdApplication + "' class='app-item'><div><img src='" + appItem.ApplicationPath + "/" + appItem.ApplicationIcon + "' />&nbsp;&nbsp;&nbsp;" + appItem.ApplicationTitle + "</div></li>");
		...
		$(".app-item").click(function()
		{
			...
			$("#menu-panel").hide(300);
			me.onApplicationIconClickEventHandler(appItem, function (runningAppItem)
			{
				me.displayRunningAppInTaskbar(me, runningAppItem);
			});
		});
